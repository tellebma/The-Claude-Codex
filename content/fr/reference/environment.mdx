...
| **Requis** | Non |
| **Défaut** | dernier modèle Sonnet de la configuration installée (vérifiez avec `claude config get model`) |
...
CLAUDE_MODEL="claude-opus-4-8" claude --print "tâche difficile"
```
...
- `claude-sonnet-5` : Équilibre recommandé entre capacités et coût
- `claude-opus-4-8` : Raisonnement le plus profond, coût le plus élevé
...
| `ANTHROPIC_MODEL` | Non | `claude-sonnet-5` | Comportement |
| `CLAUDE_MODEL` | Non | `claude-sonnet-5` | Comportement |
...
<Faq>
  ...
  <FaqItem question="Comment changer de modèle Claude Code ?">
    Définissez ANTHROPIC_MODEL avec un identifiant valide, par exemple ANTHROPIC_MODEL=claude-sonnet-5. Vous pouvez aussi le faire par session via l'argument --model.
  </FaqItem>

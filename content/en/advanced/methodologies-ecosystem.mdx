---
title: "Claude Code methodologies: Superpowers, BMAD, Spec Kit, OpenSpec"
description: "Overview of the major open-source methodologies structuring Claude Code workflows: Superpowers, Everything Claude Code, Spec Kit, BMAD, OpenSpec, gstack."
section: "advanced"
order: 13
...
| **Hedgehog** | new | BMAD planning + mechanical phase gates | Teams who don't trust unsupervised agents |
...
## Hedgehog: mechanical phase gates

Hedgehog starts from a specific worry: an agent that's told to follow a discipline will drift the moment the discipline gets in the way, unless something outside the agent enforces it. So it borrows BMAD-style planning intake, layers TDD progressively onto an opinionated TypeScript stack, and then gates every phase transition mechanically instead of trusting the agent to self-report compliance.

```
Phase 0: Planning intake (BMAD shelf) → scope, domain modules, add-ons
Phase A: Backend build, one domain module at a time
Phase B: Front-end build, same module
        ↓ mechanical gate: tests pass, coverage holds, phase checklist verified
Next module
```

Concretely, it ships as Claude Code agents and skills a project copies into its own `.claude/`: a `planner` for scoping, `backend-eng` / `front-end-eng` for the build phases, and a `reviewer` whose job is specifically to check phase and layer transitions against a checklist rather than vibe-check the diff. A **Correction Protocol** exists for when an agent skips a gate anyway — it's an admitted failure mode, not something the discipline pretends can't happen.

**Who's it for?** Teams that want BMAD-grade planning rigor but don't trust "the agent will remember to run tests" as an enforcement mechanism. Heavier than gstack, narrower than BMAD-METHOD (opinionated TypeScript stack, not stack-agnostic). Overkill for a solo prototype where nothing is gating anything yet.
...
| Don't trust the agent to self-enforce a discipline | **Hedgehog** |

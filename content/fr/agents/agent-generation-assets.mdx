...
description: "Tutoriel pour bâtir un agent Claude SDK qui génère des images via Flux/Replicate, optimise en WebP et upload. Architecture, code TypeScript et Python, coûts."
badge: "NEW"
order: 11
section: "agents"
...
La génération d'images en local (via ComfyUI ou Automatic1111) a ses avantages : tu contrôles tout, les coûts au volume sont faibles, et tu peux personnaliser les modèles. Mais elle impose un GPU dédié, un serveur toujours allumé, et une infrastructure à maintenir. Pour l'article sur ComfyUI local, voir [MCP ComfyUI local](/mcp/comfyui-mcp-local/).
...
<Step title="Boucle d'exécution de l'agent" stepNumber={4} isLast>
...
- [Panorama Claude Code + IA générative](/use-cases/ia-generative-creative/) : les quatre patterns d'intégration comparés, pour situer cet agent dans l'ensemble des approches possibles.
- [Le SDK agent en détail](/agents/agent-sdk/) : aller plus loin sur la boucle d'exécution, le streaming et la gestion d'état d'un agent Claude.

...
Votre clé `sk-ant-...` a fuité ? Révoquez-la maintenant sur [console.anthropic.com/settings/keys](https://console.anthropic.com/settings/keys), générez-en une nouvelle, mettez à jour tous les environnements (CI, local, prod), puis désactivez l'ancienne. Considérez la clé comme compromise même au moindre soupçon. La checklist détaillée est juste en dessous.

<Callout type="tip" title="Voir aussi : prévention">
Pour éviter de revivre ça, la commande [`/security-review`](/advanced/security-review) de Claude Code détecte les secrets en clair dans la diff avant le commit. Couplée à gitleaks en pre-commit, elle ferme la porte par les deux côtés.
</Callout>
...
| [1Password CLI (`op`)](https://developer.1password.com/docs/cli/) | Équipes déjà sur 1Password |
...
<Faq>
  <FaqItem question="Ma clé API Anthropic a fuité, par quoi commencer ?">
  ...
  <FaqItem question="Comment faire une rotation de clé API Anthropic en sécurité ?">
  ...
  <FaqItem question="Combien de temps prend la révocation d'une clé fuitée ?">
  ...
  <FaqItem question="Quel est l'impact financier d'une clé API Anthropic fuitée ?">
  ...
  <FaqItem question="Comment scanner mon dépôt à la recherche de clés fuitées ?">
  ...
  <FaqItem question="Faut-il réécrire l'historique git pour effacer une clé fuitée ?">

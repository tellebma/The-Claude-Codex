...
The `isolation: "worktree"` field creates a dedicated Git worktree for the agent. This is useful when running multiple agents in parallel on the same repository: each one works in its own space with no risk of conflicts.
</Callout>

...
description: "Moindre privilège, audit MCP, profils sandboxés, scan des secrets : 8 bonnes pratiques avec checklist prête à l'emploi pour sécuriser Claude Code en équipe."
badge: "Sécurité"
section: "content"
order: 8
...
Pour sécuriser Claude Code en équipe : appliquez le moindre privilège (lance depuis le plus petit dossier possible), auditez chaque MCP avant installation, utilisez des profils sandboxés par projet, et scannez les secrets via gitleaks ou trufflehog. La checklist copiable est en fin de page.

<Callout type="tip" title="Voir aussi">
La commande native [`/security-review`](/advanced/security-review) automatise la détection SQL, XSS, secrets en clair, failles d'authentification et vulnérabilités de dépendances directement dans Claude Code, en local ou en CI via GitHub Action.
</Callout>
...
<Faq>
  <FaqItem question="Quels sont les principaux risques de sécurité avec Claude Code ?">
  ...
  <FaqItem question="Comment appliquer le principe de moindre privilège à Claude Code ?">
  ...
  <FaqItem question="Comment auditer un MCP communautaire avant installation ?">
  ...
  <FaqItem question="Comment scanner les secrets dans la config Claude Code ?">
  ...
  <FaqItem question="C'est quoi un profil de config Claude Code sandboxé ?">
  ...
  <FaqItem question="Claude Code peut-il accéder à mes fichiers personnels hors projet ?">

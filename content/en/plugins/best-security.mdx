...
Top security plugins for Claude Code: **Security Reviewer** agent (code audit), **Security Review** skill (OWASP checklist), **TDD Guide** agent (tests first), **Code Reviewer** skill (quality). All Markdown files to drop into `.claude/`. Selection and install steps below.

<Callout type="tip" title="See also: the native command">
Before installing a community agent, look at the [`/security-review`](/advanced/security-review) command shipped with Claude Code. Anthropic documents it officially and already covers SQL, XSS, auth, secrets and dependencies for Pro / Max / API Console plans.
</Callout>

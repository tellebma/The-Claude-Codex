...
<Callout type="tip" title="Voir aussi">Une fois le concept de Skill compris, le méta-skill [`find-skills`](/skills/find-skills) (1.6M installs, #1 mondial sur skills.sh au 2026-05-17) permet à Claude de découvrir, vérifier et installer lui-même les skills pertinents pour votre stack.</Callout>
...
Claude Code reconnaît la commande `/review-pr` et cherche le fichier correspondant dans `.claude/commands/review-pr.md`.
</Step>
...
La variable `$ARGUMENTS` capture tout ce que vous tapez après la slash command. Par exemple, `/review-pr develop` remplace `$ARGUMENTS` par `develop`. Cela rend vos Skills dynamiques et réutilisables.
</Callout>
...
Claude Code affiche les Skills regroupés par source : built-in, projet (`.claude/commands/`) et personnel (`~/.claude/commands/`). Quelle que soit la source, chaque commande s'invoque directement par son nom (`/review-pr`).
...
- [Claude Council](/skills/claude-council) : Un pattern d'orchestration à empaqueter en Skill, pour faire délibérer plusieurs avis sur une décision difficile

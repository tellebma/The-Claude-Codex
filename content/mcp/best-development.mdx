...
- **Fix automatique** : *"Prends cette erreur Sentry, trouve la cause dans le code et corrige-la"*
...
```bash
# Linear nécessite une API key
claude mcp add linear -e LINEAR_API_KEY=lin_api_votre-clé -- npx -y @anthropic/mcp-linear
```
...
- **Planification** : *"Liste toutes les issues du backlog tagguées 'tech-debt' et estime la complexité de chacune"*
...
- **Migration** : *"Analyse le schéma actuel et génère une migration pour ajouter une table de notifications"*
...
|-----|-----------|-------------|---------------|
| **Context7** | Débutant | Aucune | Documentation à jour, éviter les hallucinations |
| **Sentry** | Intermédiaire | Auth Token | Monitoring erreurs, debug production |
| **Linear** | Intermédiaire | API Key | Gestion de projet, suivi de sprint |
| **PostgreSQL** | Intermédiaire | Connection string | Exploration et analyse de données |
| **GitHub** | Intermédiaire | PAT Token | PRs, issues, code review, CI/CD |

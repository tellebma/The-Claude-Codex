...
The sampler decides the denoising strategy: how many steps, how fast to reduce noise, which mathematical formula to use. Flux Schnell uses only 1 to 4 steps thanks to adversarial diffusion distillation, whereas SD 1.5 required 20 to 50. Fewer steps means faster generation, but results can be less detailed.
</Callout>
...
This command installs PyTorch with CUDA 12.6, the stable version recommended at the time of writing. If your GPU requires CUDA 11.8 (older architectures like Turing/Ampere with older drivers):
...
Several ComfyUI MCP projects exist on GitHub. The most actively maintained and lightest for our use case is `comfyui-mcp-server` by joenorton (308 stars, last updated May 10, 2026). It is a lightweight Python server that exposes an MCP API and delegates generation to ComfyUI via its REST API on port 8188.
...
If you prefer a more direct integration without an intermediary server, here is a minimal TypeScript wrapper that POSTs directly to the ComfyUI API. Useful when you want precise control over the JSON workflow sent:

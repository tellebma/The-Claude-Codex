...
<Callout type="tip" title="See also">Once you understand what a Skill is, the meta-skill [`find-skills`](/skills/find-skills) (1.6M installs, #1 in the world on skills.sh as of 2026-05-17) lets Claude discover, vet, and install relevant skills for your stack on its own.</Callout>
...
Claude Code recognizes the `/review-pr` command and looks for the corresponding file in `.claude/commands/review-pr.md`.
</Step>
...
The `$ARGUMENTS` variable captures everything you type after the slash command. For example, `/review-pr develop` replaces `$ARGUMENTS` with `develop`. This makes your Skills dynamic and reusable.
</Callout>
...
Claude Code displays Skills grouped by source: built-in, project (`.claude/commands/`), and personal (`~/.claude/commands/`). Whatever the source, each command is invoked directly by its name (`/review-pr`).
...
- [Claude Council](/skills/claude-council): An orchestration pattern to package as a Skill, making several opinions deliberate on a hard decision

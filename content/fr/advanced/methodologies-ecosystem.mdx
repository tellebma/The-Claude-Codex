---
title: "Méthodologies Claude Code : Superpowers, BMAD, Spec Kit, OpenSpec"
description: "Panorama des méthodologies open-source qui structurent Claude Code : Superpowers, Everything Claude Code, Spec Kit, BMAD, OpenSpec, gstack, comment les composer."
section: "advanced"
order: 13
...
| **Hedgehog** | naissant | Planning BMAD + gates de phase mécaniques | Équipes qui ne font pas confiance à l'agent sans supervision |
...
## Hedgehog : les gates de phase mécaniques

Hedgehog part d'une inquiétude précise : un agent à qui on demande de suivre une discipline s'en écarte dès qu'elle devient une gêne, à moins qu'un mécanisme extérieur à l'agent ne l'impose. La méthodologie reprend donc le planning intake façon BMAD, superpose du TDD progressif sur une stack TypeScript opinionated, puis fait passer chaque transition de phase par une porte mécanique plutôt que de faire confiance à l'agent pour s'auto-déclarer conforme.

```
Phase 0 : Planning intake (shelf BMAD) → scope, modules domaine, add-ons
Phase A : Build backend, un module domaine à la fois
Phase B : Build front-end, même module
        ↓ gate mécanique : tests passent, coverage tient, checklist de phase vérifiée
Module suivant
```

Concrètement, la méthodologie est livrée sous forme d'agents et de skills Claude Code qu'un projet copie dans son propre `.claude/` : un `planner` pour le scoping, `backend-eng` / `front-end-eng` pour les phases de build, et un `reviewer` dont le rôle est spécifiquement de vérifier les transitions de phase et de layer par rapport à une checklist plutôt que de juger le diff à l'instinct. Un **Correction Protocol** existe pour le cas où un agent saute quand même une gate — c'est un mode d'échec assumé, pas quelque chose que la discipline prétend impossible.

**Pour qui ?** Les équipes qui veulent la rigueur de planning de BMAD sans se contenter de "l'agent pensera à lancer les tests" comme mécanisme d'enforcement. Plus lourd que gstack, plus étroit que BMAD-METHOD (stack TypeScript opinionated, pas agnostique). Overkill pour un prototype solo où rien n'a encore besoin d'être gaté.
...
| Pas confiance dans l'auto-discipline de l'agent | **Hedgehog** |

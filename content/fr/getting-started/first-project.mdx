...
Trois petites choses ont changé dans le workflow de démarrage recommandé :

- **Les slash commands comme `/init`** génèrent désormais automatiquement un `CLAUDE.md` avec des valeurs par défaut sensées : plus besoin de le rédiger à la main pour un premier projet.
- **Le Plan tool** (agent intégré) est la façon canonique de cadrer avant de coder. Demandez à Claude Code de "planifier d'abord" pour tout changement qui dépasse un seul fichier.
- **Les Skills** sont livrés avec le CLI pour les tâches les plus courantes (TDD, revue de code, design frontend). Parcourez-les avec `/skills` avant de chercher une bibliothèque externe. Voir [les meilleurs Skills pour 2026](/skills/best-skills).

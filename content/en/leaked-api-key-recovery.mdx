...
Your `sk-ant-...` key was leaked? Revoke it now at [console.anthropic.com/settings/keys](https://console.anthropic.com/settings/keys), generate a new one, update every environment (CI, local, prod), then disable the old key. Treat the key as compromised on suspicion alone. The detailed checklist is right below.

<Callout type="tip" title="See also: prevention">
To avoid going through this again, Claude Code's [`/security-review`](/advanced/security-review) command detects hardcoded secrets in your diff before commit. Paired with gitleaks as a pre-commit hook, you close the door from both sides.
</Callout>
...
| [1Password CLI (`op`)](https://developer.1password.com/docs/cli/) | Teams already on 1Password |
...
<Faq>
  <FaqItem question="My Anthropic API key was leaked, what do I do first?">
  ...
  <FaqItem question="How do I rotate an Anthropic API key safely?">
  ...
  <FaqItem question="How long does it take for Anthropic to revoke a leaked key?">
  ...
  <FaqItem question="What's the cost impact of a leaked Anthropic API key?">
  ...
  <FaqItem question="How do I scan my repo for leaked API keys?">
  ...
  <FaqItem question="Should I rewrite git history to remove a leaked key?">

...
Recent changes to Claude Code configuration:

- **`ANTHROPIC_AUTH_TOKEN`** is now the recommended mechanism for enterprise gateways (proxy + Bearer token), while `ANTHROPIC_API_KEY` remains for direct use. See [Environment variables: official reference](/reference/environment).
- **`ANTHROPIC_BASE_URL`** lets you point to an internal proxy (e.g. `gateway.corp/anthropic`) without touching the rest of the config.
- **`MAX_THINKING_TOKENS`** caps the thinking budget per request: useful for bounding costs on long sessions.
- **`DISABLE_AUTOUPDATER=1`** is now deployed by default in CI to prevent a silent upgrade from breaking a running pipeline.

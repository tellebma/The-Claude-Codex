...
Si tu préfères une intégration plus directe sans serveur intermédiaire, voici un wrapper minimal en TypeScript qui POST directement sur l'API ComfyUI. Il utilise la même API `McpServer` + `server.tool()` que le tutoriel [Créer un MCP en TypeScript](/fr/mcp/create-mcp-typescript/), pour rester cohérent avec les autres exemples du site.

...
| **Required** | No |
| **Default** | the latest Sonnet model in your installed configuration (check with `claude config get model`) |
...
CLAUDE_MODEL="claude-opus-4-8" claude --print "hard task"
```
...
- `claude-sonnet-5`: Recommended balance of capability and cost
- `claude-opus-4-8`: Highest reasoning depth, highest cost
...
| `ANTHROPIC_MODEL` | No | `claude-sonnet-5` | Behaviour |
| `CLAUDE_MODEL` | No | `claude-sonnet-5` | Behaviour |
...
<Faq>
  ...
  <FaqItem question="How do I switch Claude Code to a different model?">
    Set ANTHROPIC_MODEL to a valid identifier, e.g. ANTHROPIC_MODEL=claude-sonnet-5. You can also override per-session with the --model argument.
  </FaqItem>

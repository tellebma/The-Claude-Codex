...
description: "Les 6 meilleurs Skills Claude Code 2026 : TDD Guide, Code Reviewer, Frontend Design, Plan, E2E Testing, Security Review. Installation et exemples copiables."
section: "skills"
order: 2
...
<Callout type="tip" title="Voir aussi">Cette sélection est faite à la main par la rédaction. Pour laisser Claude lui-même découvrir et installer un skill adapté à un besoin précis (sans lire 10 README), voir le méta-skill [`find-skills`](/skills/find-skills) qui interroge le leaderboard skills.sh et applique des critères qualité explicites.</Callout>
...
Les Skills présentés ici sont disponibles sous forme de fichiers Markdown à placer dans `.claude/commands/` (projet) ou `~/.claude/commands/` (personnel). Certains sont inclus dans des [extensions communautaires](/plugins/best-essential) comme **everything-claude-code** ([github.com/affaan-m/everything-claude-code](https://github.com/affaan-m/everything-claude-code)). Vous pouvez aussi les créer vous-même en vous inspirant des exemples ci-dessous.
</Callout>
...
### Skills design : la stack recommandée

Au-delà du Frontend Design intégré, plusieurs skills tiers élèvent nettement la qualité visuelle des sorties Claude Code. Notre [retour après un mois de refonte](/content/stack-design-claude-code) détaille comment les combiner avec Playwright MCP. En bref :

- **[Impeccable](/skills/impeccable)** : 23 commandes design et règles anti-patterns, Apache 2.0. Le plus court à installer pour le plus gros saut qualitatif.
- **[UI UX Pro Max](/skills/ui-ux-pro-max)** : catalogue de 67 styles et 161 palettes sur 15 stacks, MIT. Idéal pour partir d'une page blanche.
- **[Taste Skill](/skills/taste-skill)** : famille de sous-skills par registre visuel (minimaliste, brutaliste...), MIT. Pour jongler entre plusieurs styles.
- **[Huashu Design](/skills/huashu-design)** : prototypes HTML et présentations. Sous licence MIT depuis le 2026-05-14 (la licence a déjà changé une fois, re-vérifie avant un usage commercial).

Le couple à plus fort rendement si tu pars de zéro : Impeccable + Playwright MCP pour valider le rendu réel.
...
<Callout type="tip" title="Exemple concret">
Mauvaise description : "Ce skill effectue une revue de code adversariale du diff courant."

Bonne description : "Use when asked to review code, check a PR, or validate changes before merging. Activates automatically when the user mentions 'review', 'PR', or 'code quality'."
</Callout>

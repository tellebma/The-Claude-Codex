...
Top MCP risks in production: prompt injection from untrusted content, data exfiltration via tool calls, npm supply chain attacks, excessive permissions, credential theft. Mitigations: pin MCP versions, disable network access when possible, audit the source code of community MCPs before installing.

<Callout type="tip" title="See also: auditing your own code">
On the code side, Claude Code's native [`/security-review`](/advanced/security-review) command covers SQL, XSS, secrets and vulnerable dependencies in your own codebase, complementing MCP hardening on the config side.
</Callout>

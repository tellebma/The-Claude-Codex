...
The wrapper below uses the same `McpServer` + `server.tool()` pattern as the other examples on this site, detailed in [Create a TypeScript MCP Server](/mcp/create-mcp-typescript/).
...
<Step title="Configure Claude Code to use the MCP" stepNumber={4} isLast>
...
- [Local ComfyUI MCP, full tutorial](/mcp/comfyui-mcp-local/): setting up ComfyUI from scratch, installing Flux Schnell, and the initial connection to Claude Code.
- [Claude Code + generative AI overview](/use-cases/ia-generative-creative/): the four integration patterns and the decision matrix for choosing between local and cloud.

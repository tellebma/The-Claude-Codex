...
description: "Tutoriel pour exposer un workflow ComfyUI exporté en JSON via un outil MCP custom et le faire éditer dynamiquement par Claude Code (sampler, ControlNet)."
order: 13
section: "mcp"
...
Le wrapper ci-dessous utilise le même pattern `McpServer` + `server.tool()` que les autres exemples du site, présenté dans [Créer un MCP en TypeScript](/mcp/create-mcp-typescript/).
...
<Step title="Configurer Claude Code pour utiliser le MCP" stepNumber={4} isLast>
...
- [MCP ComfyUI local, tutoriel complet](/mcp/comfyui-mcp-local/) : mise en place de ComfyUI depuis zéro, installation de Flux Schnell, et connexion initiale à Claude Code.
- [Panorama Claude Code + IA générative](/use-cases/ia-generative-creative/) : les quatre patterns d'intégration et la matrice de décision pour choisir entre local et cloud.

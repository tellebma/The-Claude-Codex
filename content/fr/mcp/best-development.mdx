...
- **Token GitHub avec scopes insuffisants** : un Fine-grained token sans le scope `workflow` ne pourra pas lire les GitHub Actions. L'erreur n'est pas toujours explicite.
- **Connection string PostgreSQL** : une faute dans l'URL de connexion fait planter le MCP silencieusement. Testez-la d'abord avec `psql`.
- **Sentry org/project mal configuré** : le MCP démarre mais ne retourne rien si l'organisation ou le projet ne correspondent pas.
...
Pour découvrir de nouveaux MCP et vérifier la réputation d'un package, consultez [smithery.ai](https://smithery.ai) ou [glama.ai/mcp/servers](https://glama.ai/mcp/servers). Ces registres centralisent les évaluations de la communauté.
</Callout>
...
- [Créer un MCP custom en TypeScript](/mcp/create-mcp-typescript) : Construisez votre propre serveur MCP

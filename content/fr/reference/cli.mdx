...
|------|-------|------|-------------|
| `--model <id>` | `-m` | string | Modèle à utiliser (`claude-haiku-4-5`, `claude-sonnet-4-6`, `claude-opus-4-6`, etc.) |
...
Voir aussi la page [Background Agents](/agents/background-agents) pour l'utilisation avec Git Worktree.
...
|----------|-------------|
| `/voice` | Active/désactive le Voice Mode (push-to-talk avec Espace) |
...
| `/memory` | Affiche la mémoire persistante (CLAUDE.md) |
| `/init` | Initialise un fichier CLAUDE.md dans le projet courant |
| `/review` | Lance une revue de code du projet |
| `/cost` | Affiche le coût de la session en cours |
| `/clear` | Efface la conversation et repart à zéro |

<Callout type="info" title="/loop : tâches récurrentes">
La commande `/loop` transforme Claude Code en système de monitoring en arrière-plan. Par exemple, `/loop 10m "Vérifie si de nouvelles PRs nécessitent une revue"` vérifie toutes les 10 minutes et agit en conséquence. Par défaut, l'intervalle est de 10 minutes.
</Callout>
...
- [Voice Mode](/reference/voice-mode) : Dicter des tâches à voix haute
- [Background Agents](/agents/background-agents) : Exécution parallèle avec Git Worktree

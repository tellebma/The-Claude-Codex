...
- **Workflow par section** : (1) lister les faits précis nécessaires, (2) vérifier chacun via Playwright/WebFetch, (3) noter source + date dans un commentaire MDX masqué `{/* source: URL, consulté YYYY-MM-DD */}`, (4) rédiger uniquement avec les faits vérifiés.
- **Attention au format du commentaire** : dans un `.mdx`, il faut la syntaxe JSX `{/* ... */}`. La syntaxe HTML `<!-- ... -->` n'est pas interprétée comme un commentaire et finit rendue dans la page.
